...
./ruview.sh --band 2.4 --links 2  # capture on 2.4 GHz from two devices (remembered on the router)
./ruview.sh --calibrate           # also calibrate RuView now - only with the room empty for ~11 minutes
./ruview.sh --local               # no tunnel: nothing leaves this machine
```
...
5. **Vital signs without an empty room** — upstream RuView publishes breathing and heart rate only after a 10-minute calibration on a *completely empty* room, and afterwards only while it counts exactly one person. A router by a window, with people outside and other people at home, can never satisfy that, so the numbers never appeared. `ruview.sh` therefore applies a small local patch to RuView's server (re-applied after every update, skipped with a note if upstream changes those lines): whenever someone is detected and quality gates pass (signal quality ≥ 0.40 as in RuView; rate confidence ≥ 0.35 instead of RuView's 0.55, since live estimates here sat at 0.38–0.50 with plausible values; `RUVIEW_VITALS_MIN_CONFIDENCE` overrides it). `/api/v1/vital-signs` also lists the candidate values and gates, so an empty readout says why, the rates are published and labelled `uncalibrated_estimate` in `/api/v1/vital-signs`. They are estimates, not medical data; with several people in range they can come from any of them. When you can empty the room for about 11 minutes, `--calibrate` runs RuView's own calibration right away (you get a message when it's done; valid for 12 hours or until the script restarts), which improves its occupancy count. `RUVIEW_CALIBRATE=1` instead switches to RuView's strict mode with an announced calibration every 11.5 hours.
The Observatory's figure is placed by this project's patch, not by RuView: RuView draws it at the brightest cell of a synthetic signal map rebuilt ~94 times a second from whichever link reported last, so it jumped metres back and forth, and that position isn't a measurement (its own code says one link can't resolve a room position). Here it stands between the two links and leans, smoothed over ~2.5 s, toward the one with more movement, and breathing/heart rate are hidden whenever RuView reports nobody present. With 2 links that's as much position as the data holds; RuView's docs recommend 3–6+ dedicated nodes for real localisation.
